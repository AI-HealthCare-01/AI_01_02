...
export default function AppLayout() {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      {/* ── Main content ── */}
      <main className="flex-1 overflow-y-auto pb-28 pt-[4.5rem] md:pb-0 md:pt-0">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 flex min-h-[88px] border-t border-gray-200/40 bg-white/92 px-1 pt-3 shadow-[0_-12px_30px_rgba(15,23,42,0.08)] backdrop-blur-xl pb-[calc(env(safe-area-inset-bottom)+0.8rem)]">
        {PRIMARY_NAV_ITEMS.map(({ to, label, icon: Icon, end }) => (
          <NavLink
            key={to}
            to={to}
            ...
            className={({ isActive }) =>
              `relative flex flex-1 flex-col items-center justify-center gap-1 rounded-2xl py-3 transition-all duration-200 ${

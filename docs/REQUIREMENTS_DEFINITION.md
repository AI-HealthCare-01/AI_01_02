...
문서 변경 이력:
- v1.10 (2026-02-24): OCR 원본 즉시 폐기 경로 보강(큐 등록 실패 포함) 및 좌표(raw blocks) 선택 저장 정책 반영
- v1.9 (2026-02-24): `알약 이미지 분류 기반 복약 분석` 기능 범위 제외 반영(시스템 로직/요구사항/검수 기준/데이터 섹션 정리)
...
- v1.6 (2026-02-23): OCR 원본 이미지 즉시 폐기 정책, 챗봇 고도화(하이브리드 검색, 출처 표기, 저유사도 재질문, 세션 자동 종료) 반영
- v1.5 (2026-02-23): 객체 모델/ API 계약 명세(기존 11~12장)를 `docs/API_SPECIFICATION.md`로 분리
- v1.0 (2026-02-20): OCR/가이드/기본 챗봇/알림 중심 초기 정의
- v1.1 (2026-02-23): 실시간 챗봇(RAG+SSE) 요구사항 상세화
...
- 일반 사용자: 회원가입, 건강 프로필 입력, 문서 업로드, OCR 결과 확인/수정, 가이드 조회, 챗봇 사용
...
6. 원본 업로드 이미지는 OCR/파싱 처리 완료 직후 즉시 폐기하며 DB/클라우드 스토리지에 파일 형태로 저장하지 않는다.
   큐 등록 실패 등으로 비동기 처리가 시작되지 못한 경우에도 원본 파일은 즉시 폐기한다.
...
| REQ-009 | 기능 | OCR | OCR 결과 저장 | AI 워커는 OCR 성공 시 raw 텍스트와 구조화 데이터를 DB에 저장해야 하며, 좌표(raw blocks)는 저신뢰/검수 필요 케이스 중심으로 선택 저장할 수 있어야 한다. | High | 실패 시 error_message 저장 |
...
| ID | 구분 | 카테고리 | 요구사항 명칭 | 상세 내용 | 우선순위 | 비고 |
|---|---|---|---|---|---|---|
| REQ-101 | 비기능 | 성능 | 핵심 응답 지연 | 챗봇 첫 토큰(TTFT)은 P95 3초 이내를 목표로 한다. | High | 평가 기준 반영 |
...
| REQ-110 | 비기능 | 확장성 | 구조 확장성 | OCR/가이드/챗봇 모델 교체 시 API 계약 변경 없이 워커 레이어에서 교체 가능해야 한다. | Medium | 추후 고도화 대비 |
| REQ-111 | 비기능 | 데이터 | 저장 포맷 표준화 | 문서, OCR 결과, 가이드, 채팅 데이터는 UTF-8과 JSON 스키마를 일관 사용해야 한다. | Medium | 한글 깨짐 방지 |
...
| REQ-117 | 비기능 | 성능 | 비동기 논블로킹 처리 | 챗봇 스트리밍 API는 이벤트 루프 블로킹을 최소화해야 한다. | High | 워커 오프로딩 병행 |
...
- OCR 원문 저장:
  - 기본 저장: raw 텍스트 + 구조화 결과 + 엔진 메타데이터
  - 선택 저장: 텍스트 블록 + 좌표(Bounding Box) (저신뢰/검수 필요 케이스 우선)
- OCR 원본 이미지 보관 정책:
...
- 미구현 또는 고도화 필요:
  ...
  - 프라이버시/확장성/안전성 보강: REQ-113~115, REQ-117~125
...
- 기능 요구사항 High 항목(REQ-001~013, REQ-014~016, REQ-024~031, REQ-039~042, REQ-044, REQ-046~056, REQ-058, REQ-060)이 구현되었는가
- 비기능 요구사항 High 항목(REQ-101~109, REQ-113~114, REQ-117~124, REQ-126~127)이 측정 가능하게 검증되었는가
- 선택 기능(REQ-017~023, REQ-032, REQ-043, REQ-045, REQ-057, REQ-059, REQ-061, REQ-110~112, REQ-115, REQ-125)은 반영 여부와 사유가 기록되었는가
